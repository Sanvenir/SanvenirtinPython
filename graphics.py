...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window", width=200, height=200, autoflush=True, positionX = 100, positionY = 100):
        windows.append(self)
        master = tk.Toplevel(_root)
        master.protocol("WM_DELETE_WINDOW", self.close)
        tk.Canvas.__init__(self, master, width=width, height=height)
        master.geometry( '%dx%d+%d+%d'%(width, height, positionX, positionY))
